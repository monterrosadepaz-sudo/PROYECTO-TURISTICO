...
export default function TablaPropuestas({ propuestas, tipo, alAnalizar, alAccionar, usuarios }) {
  ...
  // MODO USUARIOS: Preparado para el flujo de edición selectiva
  if (tipo === 'usuarios') {
    return (
              ...
              <tr key={u.idusuario} className="bg-slate-50/50 hover:bg-white transition-all group rounded-2xl">
                ...
                <td className="px-6 py-4 last:rounded-r-2xl border-y border-r border-transparent group-hover:border-slate-100 text-right">
                  <div className="flex justify-end gap-2">
                    {/* ENVIAMOS EL OBJETO 'u' COMPLETO: Esto activa la selección en el AdminDashboard */}
                    <button 
                    ...
                    {/* PROTECCIÓN LA LN: Bloqueo de eliminación para administradores */}
          ...
          {propuestas.map((sitio) => (
            <tr key={sitio.id} className="bg-slate-50/50 hover:bg-white transition-all group rounded-2xl">
              ...
              <td className="px-6 py-4 last:rounded-r-2xl border-y border-r border-transparent group-hover:border-slate-100 text-right">
                <div className="flex justify-end gap-2">
                  {tipo === 'pendientes' && (
                    <button onClick={() => alAnalizar(sitio)} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shadow-lg shadow-blue-100">
                      Analizar Propuesta
                    </button>
                  )}
                  {tipo === 'activos' && (
