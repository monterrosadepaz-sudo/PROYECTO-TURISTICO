...
export default function TablaPropuestas({ propuestas, tipo, alAnalizar, alAccionar, usuarios }) {
  ...
  // MODO USUARIOS (Tus líneas originales respetadas)
  if (tipo === 'usuarios') {
    return (
  ...
  // MODO PROPUESTAS (Actualizado con llaves de Julio: idprefomulario, nombre, imagen)
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-left border-separate border-spacing-y-3">
        <thead>
        ...
        <tbody>
          {propuestas.map((sitio) => (
            <tr key={sitio.idprefomulario} className="bg-slate-50/50 hover:bg-white transition-all group rounded-2xl">
              <td className="px-6 py-4 first:rounded-l-2xl border-y border-l border-transparent group-hover:border-slate-100">
                <div className="flex items-center gap-3">
                  {/* Imagen desde el storage de Julio */}
                  <img 
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="text-[8px] font-black text-blue-600/40 uppercase italic">${sitio.costo_entrada?.toFixed(2)}</span>
                  </div>
                </div>
              </td>
              <td className="px-6 py-4 border-y border-transparent group-hover:border-slate-100">
              ...
              <td className="px-6 py-4 last:rounded-r-2xl border-y border-r border-transparent group-hover:border-slate-100 text-right">
                <div className="flex justify-end gap-2">
                  {tipo === 'pendientes' && (
                    <>
                      <button 
                        ...
                        className="text-blue-600 hover:bg-blue-50 px-4 py-2 rounded-xl text-[10px] font-black uppercase italic tracking-widest transition-all"
                      >
                        Analizar
                      </button>
                      <button 
                        onClick={() => alAccionar(sitio.idprefomulario, 'aprobar_propuesta')} 
                        className="bg-green-600 hover:bg-green-700 text-white px-5 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shadow-lg shadow-green-100"
                      >
                        Aprobar
                      </button>
                      <button 
                        onClick={() => alAccionar(sitio.idprefomulario, 'eliminar_propuesta')} 
                        className="bg-red-50 text-red-600 hover:bg-red-100 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all"
                      >
                        Borrar
                      </button>
                    </>
                  ...
                  {tipo === 'activos' && (
                    <button onClick={() => alAccionar(sitio.idprefomulario, 'ocultar')} className="border-2 border-slate-200 text-slate-400 hover:border-amber-400 hover:text-amber-500 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all">
